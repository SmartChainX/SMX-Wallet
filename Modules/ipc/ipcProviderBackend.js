/**
The IPC provider backend filter and tunnel all incoming request to the chainx node.
...
const chainxNode = require('../chainxNode');
...
class IpcProviderBackend {
    constructor() {
       ...
       chainx.on('state', _.bind(this._onNodeStateChanged, this));
            ...
            if (!socket.isConnected) {
                ...
                if (!this._connectionPromise[ownerId]) {
                    this._connectionPromise[ownerId] = Q.try(() => {
                        ...
                        if (chainxNode.state !== chainx.STATES.CONNECTED) {
                            return new Q((resolve, reject) => {
                                const onStateChange = (newState) => {
                                    if (chainx.STATES.CONNECTED === newState) {
                                        chainx.removeListener('state', onStateChange);

                                        log.debug(`chainx node connected, resume connecting socket ${ownerId}`);

                                        resolve();
                                    }
                                };

                                chainx.on('state', onStateChange);
                            });
                        }
                    })
                    .then(() => {
    ...
    /**
     * Handler for when ChainX node state changes.
     *
     * Auto-reconnect sockets when chainx node state changes
     *
    ...
    _onNodeStateChanged(state) {
        ...
        case chainxNode.STATES.STOPPING:
            log.info('ChainX node stopping, disconnecting sockets');
    ...
    */
    _sanitizeRequestPayload(conn, payload) {
        if (_.isArray(payload)) {
            _.each(payload, (p) => {
                if (p.method === 'chx_sendTransaction') {
