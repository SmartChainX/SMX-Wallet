...
const config = JSON.parse(
    ...
    if (manager.clients.state.available) {
       Path = manager.clients.activeCli.fullPath;
    }


    const chx = chxPrivate({
        chxPath,
        balance: 5,
        genesisBlock: {
            config: {
...
exports.mocha = (_module, options) => {
    ...
    _module.exports[options.name || path.basename(_module.filename)] = {
        * before() {
            ...
            const appFileName = (options.app === 'wallet') ? 
            const appVers = packageJson.version.replace(/\./ig, '-');
            const platformArch = `${process.platform}-${process.arch}`;

            let appPath;
            let ipcProviderPath = path.join(this.dataDir, 'ipc');
